...
def _create_red_image_part(prop):
    imarray = numpy.asarray([(__propFilter(redpart,prop),0,0) for redpart in numpy.random.rand(64*64)*255])
    imarray.shape=(64,64,3)
    return imarray.astype('uint8')

def _create_green_image_part(prop):
    imarray = numpy.asarray([(0,__propFilter(greenpart,prop),0) for greenpart in numpy.random.rand(64*64)*255])
    imarray.shape=(64,64,3)
    return imarray.astype('uint8')

def _create_blue_image_part(prop):
    imarray = numpy.asarray([(0,0,__propFilter(bluepart,prop)) for bluepart in numpy.random.rand(64*64)*255])
    imarray.shape=(64,64,3)
    return imarray.astype('uint8')
